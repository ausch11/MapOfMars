...
class Pooling(nn.Module):
    ...
    def __init__(self, dim, pool_size=3):
        super().__init__()
        self.depthwise = nn.Conv2d(dim, dim, kernel_size=pool_size, stride=1,
                                   padding=pool_size // 2, groups=dim)
...
class Mlp(nn.Module):
    ...
    def forward(self, x):
        ...
        x = self.msdc(x)
        x = torch.cat(x, dim=1)
        x = self.multi_granular_shuffle(x)  # 多粒度层次化混洗
        # x = self.das(x)
        # ori
        # group =  gcd(self.combined_channels, self.hidden_features) #128
        # x = channel_shuffle(x, group)
...
class MultiScaleBlock(BaseModule):
    ...
    def __init__(
        ...
        pool_size = 3
        self.token_mixer = Pooling(attn_dims, pool_size=pool_size)

        drop = 0.
        self.mlp = Mlp(
            in_features=attn_dims,
            hidden_features=int(attn_dims * mlp_ratio),
            act_cfg=act_cfg,
            drop=drop)
        ...
        self.layer_scale_1 = nn.Parameter(
            layer_scale_init_value * torch.ones((attn_dims)), requires_grad=True)
        self.layer_scale_2 = nn.Parameter(
            layer_scale_init_value * torch.ones((attn_dims)), requires_grad=True)

        self.local_unit = DynamicConv2d(
            dim=attn_dims, kernel_size=7, num_groups=2)

        self.channel_adjust = None
        if in_dims != attn_dims:
            self.channel_adjust = nn.Conv2d(in_dims, attn_dims, kernel_size=1)
    ...
    def forward(self, x, in_size):
        ...
        B, _, C = x_norm.shape
        H = in_size[0]
        x_norm = x_norm.reshape(B, C, in_size[0], in_size[1])

        # 调整通道数（如果存在调整层）
        if self.channel_adjust is not None:
            x_norm = self.channel_adjust(x_norm)  # 自动匹配设备
            B, C, H, W = x_norm.shape

        x_pool = self.token_mixer(x_norm)
        x_pool = self.layer_scale_1.unsqueeze(-1).unsqueeze(-1) * x_pool
        x_conv = self.local_unit(x_norm)


        if C != self.attn_dims:
            x = self.channel_adjust(x)  # (B, attn_dims, H, W)
            x = x.permute(0, 2, 1).reshape(B, _, H, H)
            x = x + self.drop_path(x_pool + x_conv)

        else:
            x = x.reshape(B, C, H, H) + self.drop_path(x_pool + x_conv)

        x = x + self.drop_path(
            self.layer_scale_2.unsqueeze(-1).unsqueeze(-1) *
            self.mlp(self.norm2(x.reshape(B, _, C)).reshape(B, C, H, H)))
