...
class TransformerEncoderLayer(BaseModule):
    ...
    def forward(self, x):
        ...
        # 2. 提取图像块特征（移除分类标记）
        patch_features = x1[:, 1:, :]  # [4, 196, 768]

        # 3. 转换为空间格式 [B, C, H_patches, W_patches]
        H_patches = int(patch_features.shape[1] ** 0.5)  # 14
        spatial_features = patch_features.reshape(
            B, H_patches, H_patches, C
        ).permute(0, 3, 1, 2)  # [4, 768, 14, 14]
        ...
        upsampled_features = self.attn(upsampled_features)

        processed_spatial = upsampled_features

        # 还原
        # 1. 下采样回块特征维度（若需要）
        # 如果直接使用未上采样的特征，跳过此步
        downsampled_features = F.adaptive_avg_pool2d(
            processed_spatial, (H_patches, H_patches)
        )  # [4, 768, 14, 14]

        # 2. 转换为序列格式 [B, N, C]
        # 调整维度顺序并展平
        patch_features_recovered = downsampled_features.permute(0, 2, 3, 1).reshape(
            B, H_patches * H_patches, C
        )  # [4, 196, 768]

        # 3. 重新添加分类标记（从原始输入或模型中获取）
        # 这里假设 cls_token 已保留或通过模型参数获取
        output_sequence = torch.cat(
            [cls_token, patch_features_recovered],
            dim=1
        )  # [4, 197, 768]
...
class VisionTransformer_marsmapformer(BaseBackbone):
    """Vision Transformer.
    ...
    def __init__(self,
                 ...
                 pre_norm=False,
                 init_cfg=None):
        super(VisionTransformer_marsmapformer, self).__init__(init_cfg)
    ...
    def init_weights(self):
        super(VisionTransformer_marsmapformer, self).init_weights()
