...
def TCLoss(y_pred, y_true, class_weight=None):
    # y_pred=48,15 y_true 48,
    loss = -y_true * F.log_softmax(y_pred, dim=-1)
    smooth = 1e-6
    y = (1 - y_true) * torch.log(1 - y_pred + smooth)

    x1 = y_pred + smooth  # 48,15
    x2 = torch.log(x1)  # 48,15
    x = y_true * x2
    # y = (1 - y_true) * torch.log(1 - y_pred + smooth)
    loss = -torch.mean(x + y)
